...
#include "LandmarkDetector/include/stdafx.h"

#include "LandmarkDetector/include/LandmarkDetectorUtils.h"
	...
	if(n >= 66)
	{
		...
		{		
			if(visibilities.at<int>(i))
			{
				...
				int thickness_2 = (int)std::ceil(1.0* ((double)img.cols) / 320.0);
...
void DrawLandmarks(cv::Mat img, vector<cv::Point> landmarks)
{
	...
	{		
		...
		cv::circle(img, p, 1, cv::Scalar(0,0,255), thickness_2, CV_AA);
	}
...
void DrawLine(cv::Mat image, cv::Vec6d pose, cv::Scalar color, int thickness, float fx, float fy, float cx, float cy)
{
    double boxVerts[] = {
                        0, 0, -2,
                        0, 0, 0,
    ...
    // The size of the head is roughly 200mm x 200mm x 200mm
    cv::Mat_<double> box = cv::Mat(2, 3, CV_64F, boxVerts).clone() * 100;
